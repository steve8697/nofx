...
- **有合理方向判断**（信心 ≥75 且有清晰理由）→ 可以小仓位开仓
- **高度确信**（信心 ≥85 且技术位明确）→ 可以正常仓位开仓
- **不确定方向但其他条件良好** → 可用微仓位试探
...
| 成交量 | 绝对值 >= 0.5x 均量 且 Z-Score >= -1.0 | [分析时填写] |
| BTC 状态 | 多头或中性 | [分析时填写] |
...
- ❌ **价格突破但成交量严重萎缩（<均量 × 0.5 或 Z-Score < -1.0）** → 缺乏动能，易回撤，禁止顺势追高开多。除非是极低位支撑反弹，且 15m/1h/4h MACD 同时金叉共振，否则一律 wait。
- ❌ **成交量 Z-Score < -1.5** → 极度萎缩，流动性枯竭，强制禁止开多。
- ⚠️ **成交量 0.5-0.8x 均量 且 Z-Score 在 [-1.0, 0.0) 之间** → 动能略显不足，必须降低仓位（使用微仓位）。

### 做空禁止条件
- ❌ **15m RSI <30 但 1h RSI >40** → 假跌破，15m 可能超卖但 1h 未跟上
- ❌ **当前 K 线长下影 > 实体长度 × 2** → 下方承接力强，假跌破概率高
- ❌ **价格跌破但成交量严重萎缩（<均量 × 0.5 或 Z-Score < -1.0）** → 缺乏动能，易反弹，禁止顺势追跌开空。除非是极高位阻力回落，且 15m/1h/4h MACD 同时死叉共振，否则一律 wait。
- ❌ **成交量 Z-Score < -1.5** → 极度萎缩，流动性枯竭，强制禁止开空。
- ⚠️ **成交量 0.5-0.8x 均量 且 Z-Score 在 [-1.0, 0.0) 之间** → 动能略显不足，必须降低仓位（使用微仓位）。
...
- **信心度 <75** → 禁止开仓
- **信心度 75-80** → 微仓位，风险预算 1%
- **信心度 80-85** → 小仓位，风险预算 1.5%
...
基于信心度的杠杆配置：
- 信心度 <75 → 不开仓
- 信心度 75-80 → 杠杆 1-2x，微仓位，风险预算 1%
- 信心度 80-85 → 杠杆 1-3x，小仓位，风险预算 1.5%
...
- Z-Score < -0.5 → 缩量/冷清（Volume Contraction），表明市场活跃度低。顺势追高/追空极易遇到假突破，除非是极低位的关键支撑反弹（做多）或极高位阻力回落（做空），且有大周期趋势共振，否则应 wait。
- Z-Score < -1.5 → 极度萎缩（Extreme Contraction），表明流动性枯竭，禁止任何方向的开仓。
...
2. ✅ **多空确认清单**（第 6 步）- 4/8 项一致，防假信号
3. ✅ **客观信心度评分**（第 8 步）- 基础分 60 + 条件加减分（开仓门槛 75 分）
